def it_was_ok
  ...
  Movie.where('score BETWEEN ? AND ?', 2,3).select(:id, :title, :score)
...
def harrison_ford
  ...
  # as a lead actor.
  Movie.select('movies.id','movies.title').joins(:actors).where('(castings.ord != ? AND actors.name = ?)', 1, 'Harrison Ford')
...
def biggest_cast
  ...
  # actors).
  Movie.select('movies.id', 'movies.title').joins(:castings).group('movies.id').limit(3).order("COUNT(castings.movie_id) DESC")
end
